...
def load_all_batches(source_folder, num_batches):
    ...
    Expected file names: processed_images_batch_{i}.npy and processed_labels_batch_{i}.npy
    """
    all_images = []
    all_labels = []
...
def preprocess_images(images, target_size=(224, 224)):
    """
    Preprocess images:
      - Convert grayscale images to 3-channel if necessary.
    ...
    processed = []
    for img in images:
        # Convert grayscale (2D) or single-channel images to 3 channels.
        if img.ndim == 2:
...
def build_model(num_classes):
    """
    Build a DenseNet-based classifier.
    """
    base_model = DenseNet121(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
    base_model.trainable = False  # Freeze the DenseNet base.
    model = Sequential([
        base_model,
...
def main():
    # Folder containing your filtered images and labels.
    source_folder = '/Users/issackondreddy/Desktop/Programming/ML Project/Lung Cancer Detection/data/processed_images_by_filter'
    # Set the total number of batches (adjust as needed; here, e.g., batches 0 to 9).
    num_batches = 10
    ...
    model_save_path = os.path.join(save_folder, 'densenet_classifier_filter.h5')
